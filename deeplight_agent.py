...
class DeeplightAgent(NetworkAgent):
    ...
    def build_network(self):
        ...
        dic_input_node = {}
        for feature_name in self.para_set.LIST_STATE_FEATURE:
            dic_input_node[feature_name] = Input(
        ...
        for feature_name in self.para_set.LIST_STATE_FEATURE:
            if len(getattr(State, "D_" + feature_name.upper())) > 1:
                dic_flatten_node[feature_name] = self._cnn_network_structure(
                    dic_input_node[feature_name]
                )
            else:
        ...
        all_flatten_feature = concatenate(
            list_all_flatten_feature, axis=1, name="all_flatten_feature"
        )

        # shared dense layer
        shared_dense = self._shared_network_structure(
            all_flatten_feature, self.para_set.D_DENSE
        )

        # build phase selector layer
        if (
            "cur_phase" in self.para_set.LIST_STATE_FEATURE
            and self.para_set.PHASE_SELECTOR
        ):
            list_selected_q_values = []
            for phase in range(self.num_phases):
                locals()["q_values_{0}".format(phase)] = (
                    self._separate_network_structure(
                        shared_dense,
                        self.para_set.D_DENSE,
                        self.num_actions,
                        memo=phase,
                    )
                )
                locals()["selector_{0}".format(phase)] = Selector(
                    phase, name="selector_{0}".format(phase)
                )(dic_input_node["cur_phase"])
                locals()["q_values_{0}_selected".format(phase)] = Multiply(
                    name="multiply_{0}".format(phase)
                )(
                    [
                        locals()["q_values_{0}".format(phase)],
                ...
                list_selected_q_values.append(
                    locals()["q_values_{0}_selected".format(phase)]
                )
            q_values = Add()(list_selected_q_values)
        else:
            q_values = self._separate_network_structure(
                shared_dense, self.para_set.D_DENSE, self.num_actions
            )
        ...
        network.compile(
            optimizer=RMSprop(learning_rate=self.para_set.LEARNING_RATE),
        ...
        if self.para_set.SEPARATE_MEMORY:
            ...
            for phase_i in range(self.num_phases):
                for action_i in range(self.num_actions):
                    ...
                    if (
                        len(self.memory[phase_i][action_i])
                        > self.para_set.MAX_MEMORY_LEN
                    ):
                        print(
                            "length of memory (state {0}, action {1}): {2}, before forget".format(
                                phase_i, action_i, len(self.memory[phase_i][action_i])
                            )
                        )
                        self.memory[phase_i][action_i] = self.memory[phase_i][action_i][
                            -self.para_set.MAX_MEMORY_LEN :
                        ]
                    print(
                        "length of memory (state {0}, action {1}): {2}, after forget".format(
                            phase_i, action_i, len(self.memory[phase_i][action_i])
                        )
                    )
        else:
    ...
    def get_sample(
        self, memory_slice, dic_state_feature_arrays, Y, gamma, prefix, use_average
    ):

        len_memory_slice = len(memory_slice)

        f_samples = open(
            os.path.join(self.path_set.PATH_TO_OUTPUT, "{0}_memory".format(prefix)), "a"
        )
    ...
    def update_network(self, if_pretrain, use_average, current_time):
        ...
        Xs = [
            np.array(dic_state_feature_arrays[feature_name])
            for feature_name in self.para_set.LIST_STATE_FEATURE
        ]
        Y = np.array(Y)
    ...
    def _cal_priority(sample_weight):
        ...
        sample_weight_np = (
            np.power(sample_weight_np + pos_constant, alpha) / sample_weight_np.sum()
        )
