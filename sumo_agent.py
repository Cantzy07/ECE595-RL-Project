...
class SumoAgent:
    ...
    def __init__(self, sumo_cmd_str, path_set):
        ...
        self.f_log_rewards = os.path.join(
            self.path_set.PATH_TO_OUTPUT, "log_rewards.txt"
        )
        if not os.path.exists(self.f_log_rewards):
            f = open(self.f_log_rewards, "w")
            list_reward_keys = np.sort(
                list(self.para_set.REWARDS_INFO_DICT.keys())
    ...
    def take_action(self, action):
        ...
        if (
            ...
            < self.para_set.MIN_PHASE_TIME[current_phase_number]
        ):
            action = 0
        for i in range(self.para_set.MIN_ACTION_TIME):
            ...
            if action == 1 and i == 0:
                action_in_second = 1
            self.current_phase, self.current_phase_duration, self.vehicle_dict = (
                map_computor.run(
    ...
    def update_state(self):
        ...
        self.state = State(
            ...
            map_feature=np.reshape(
                np.array(status_tracker[3]), newshape=(1, 150, 150, 1)
            ),
            cur_phase=np.reshape(np.array([self.current_phase]), newshape=(1, 1)),
            next_phase=np.reshape(
                np.array(
                    [(self.current_phase + 1) % len(self.para_set.MIN_PHASE_TIME)]
                ),
                newshape=(1, 1),
            ),
            time_this_phase=np.reshape(
                np.array([self.current_phase_duration]), newshape=(1, 1)
            ),
            if_terminal=False,
        )
    ...
    def cal_reward_from_list(self, reward_detail_dict_list):
        ...
        return reward * (1 - 0.8)  # the gamma value is 0.8
