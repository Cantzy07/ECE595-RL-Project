...
def find_surrounding_lane_WESN(
    central_node_id="node0", WESN_node_ids={"W": "1", "E": "2", "S": "3", "N": "4"}
):
    tree = ET.parse("./data/cross.net.xml")
    root = tree.getroot()
    four_lane_ids_dict = {}
    for k, v in WESN_node_ids.items():
        four_lane_ids_dict[k] = root.find(
            "./edge[@from='%s'][@to='%s']" % (v, central_node_id)
        ).get("id")
...
def get_phase_affected_lane_traffic_max_volume(
    phase="NSG_SNG",
    tl_node_id="node0",
    WESN_node_ids={"W": "1", "E": "2", "S": "3", "N": "4"},
):
    four_lane_ids_dict = find_surrounding_lane_WESN(
        central_node_id=tl_node_id, WESN_node_ids=WESN_node_ids
    )
    directions = phase.split("_")
    traffic_volume_start_end = []
    for direction in directions:
        traffic_volume_start_end.append(
            [four_lane_ids_dict[direction[0]], four_lane_ids_dict[direction[1]]]
        )
    ...
    for lane_id in traffic_volume_start_end:
        ...
        time_begin = root.find(
            "./flow[@from='%s'][@to='%s']" % (lane_id[0], to_lane_id)
        ).get("begin")
        time_end = root.find(
            "./flow[@from='%s'][@to='%s']" % (lane_id[0], to_lane_id)
        ).get("end")
...
def phase_affected_lane_position(
    phase="NSG_SNG",
    tl_node_id="node0",
    WESN_node_ids={"W": "1", "E": "2", "S": "3", "N": "4"},
):
    ...
    for lane_id in affected_lanes:
        lane_shape = root.find(
            "./edge[@to='%s']/lane[@id='%s']" % (tl_node_id, lane_id)
        ).get("shape")
        ...
        ind_x1, ind_x2, ind_y1, ind_y2 = coordinate_mapper(
...
def phases_affected_lane_postions(
    phases=["NSG_SNG_NWG_SEG", "NEG_SWG_NWG_SEG"],
    tl_node_id="node0",
    WESN_node_ids={"W": "1", "E": "2", "S": "3", "N": "4"},
):
    parameterArray = []
    for phase in phases:
        parameterArray += phase_affected_lane_position(
            phase=phase, tl_node_id=tl_node_id, WESN_node_ids=WESN_node_ids
        )
...
def getMapOfCertainTrafficLight(curtent_phase=0, tl_node_id="node0", area_length=600):
    ...
    for affected_road in parameterArray:
        resultTrained[
            affected_road[0] : affected_road[1], affected_road[2] : affected_road[3]
        ] = 127
...
def calculate_reward(tempLastVehicleStateList):
    ...
    for key, vehicle_dict in tempLastVehicleStateList.items():
        ...
        if (
            tempLastVehicleStateList[key]["former_speed"] > 0.5
            and tempLastVehicleStateList[key]["speed"] < 0.5
        ):
            stop_count += (
                tempLastVehicleStateList[key]["stop_count"]
                - tempLastVehicleStateList[key]["former_stop_count"]
            )
    # PI = (waitedTime + 10 * stop_count) / len(tempLastVehicleStateList) if len(tempLastVehicleStateList)!=0 else 0
    PI = (
        waitedTime / len(tempLastVehicleStateList)
        if len(tempLastVehicleStateList) != 0
        else 0
    )
    return -PI
...
def log_rewards(
    ...
    reward, reward_detail_dict = get_rewards_from_sumo(
        vehicle_dict, action, rewards_info_dict
    )
...
def get_rewards_from_sumo(
    ...
    reward_detail_dict["duration"].append(
        get_travel_time_duration(vehicle_dict, vehicle_id_entering_list)
    )
    ...
    reward_detail_dict["num_of_vehicles_at_entering"] = [
        False,
        0,
        len(vehicle_id_entering_list),
    ]
...
def get_lane_queue_variance(listLanes):
    queue_variance = 0
    if len(listLanes) < 2:
        return queue_variance
    n = mean = m2 = 0.0
    for lane in listLanes:
        n += 1
        val = traci.lane.getLastStepHaltingNumber(lane)
        delta = val - mean
        mean += delta / n
        delta2 = val - mean
        m2 += delta * delta2
...
def get_lane_waiting_variance(listLanes):
    waiting_variance = 0
    if len(listLanes) < 2:
    ...
    for lane in listLanes:
        n += 1
        val = traci.lane.getWaitingTime(str(lane)) / 60.0
        delta = val - mean
        mean += delta / n
        delta2 = val - mean
        m2 += delta * delta2
    waiting_variance = m2 / n
    return waiting_variance
...
def get_overall_delay(listLanes):
    overall_delay = 0
    for lane in listLanes:
        overall_delay += 1 - traci.lane.getLastStepMeanSpeed(
            str(lane)
        ) / traci.lane.getMaxSpeed(str(lane))
    # TODO:log output
    return overall_delay


def get_lane_delay_variance(listLanes):
    delay_variance = 0
    if len(listLanes) < 2:
    ...
    for lane in listLanes:
        n += 1
        val = 1 - traci.lane.getLastStepMeanSpeed(str(lane)) / traci.lane.getMaxSpeed(
            str(lane)
        )
        delta = val - mean
        mean += delta / n
...
def get_partial_travel_time_duration(vehicle_dict, vehicle_id_list):
    ...
    for vehicle_id in vehicle_id_list:
        if (vehicle_id in vehicle_dict.keys()) and (
            vehicle_dict[vehicle_id].first_stop_time != -1
        ):
            travel_time_duration += (
                traci.simulation.getTime() / 1000
                - vehicle_dict[vehicle_id].first_stop_time
            ) / 60.0
...
def get_travel_time_duration(vehicle_dict, vehicle_id_list):
    ...
    for vehicle_id in vehicle_id_list:
        if vehicle_id in vehicle_dict.keys():
            travel_time_duration += (
                traci.simulation.getTime() / 1000 - vehicle_dict[vehicle_id].enter_time
            ) / 60.0
    if len(vehicle_id_list) > 0:
        return travel_time_duration  # /len(vehicle_id_list)
    else:
...
def update_vehicles_state(dic_vehicles):
    ...
    for vehicle_id in vehicle_id_list:
        if (vehicle_id in dic_vehicles.keys()) == False:
            ...
            dic_vehicles[vehicle_id].speed = traci.vehicle.getSubscriptionResults(
                vehicle_id
            ).get(64)
            if (dic_vehicles[vehicle_id].speed < 0.1) and (
                dic_vehicles[vehicle_id].first_stop_time == -1
            ):
                dic_vehicles[vehicle_id].first_stop_time = (
                    traci.simulation.getTime() / 1000
                )
...
def get_vehicle_id_leaving(vehicle_dict):
    ...
    for vehicle_id in vehicle_dict.keys():
        if (
            not (vehicle_id in vehicle_id_entering)
            and vehicle_dict[vehicle_id].entering
        ):
...
def run(
    ...
    if action == 1:
        ...
        )  # change traffic light in SUMO according to actionToPerform
...
def phase_vector_to_number(phase_vector, phases_light=phases_light_7):
    ...
    if phase_vector in phase_vector_7:
        ...
        raise (
            "Phase vector %s is not in phases_light %s"
            % (phase_vector, str(phase_vector_7))
        )
