...
class Selector(Layer):
    ...
    def __init__(self, select, **kwargs):
        ...
        self.select = select
        self.select_neuron = tf.constant(value=self.select)
    ...
    def call(self, x):
        sel = tf.cast(self.select_neuron, tf.float32)
        return tf.cast(tf.equal(x, sel), tf.float32)
...
def conv2d_bn(input_layer, index_layer, filters=16, kernel_size=(3, 3), strides=(1, 1)):
    ...
    conv = Conv2D(
        ...
        padding="same",
        use_bias=False,
        name="conv{0}".format(index_layer),
    )(input_layer)
    bn = BatchNormalization(
        axis=bn_axis, scale=False, name="bn{0}".format(index_layer)
    )(conv)
...
class NetworkAgent(Agent):
    ...
    def _cnn_network_structure(img_features):
        conv1 = conv2d_bn(
            img_features, 1, filters=32, kernel_size=(8, 8), strides=(4, 4)
        )
    ...
    def _shared_network_structure(state_features, dense_d):
        hidden_1 = Dense(dense_d, activation="sigmoid", name="hidden_shared_1")(
            state_features
        )
        return hidden_1
    ...
    def _separate_network_structure(state_features, dense_d, num_actions, memo=""):
        hidden_1 = Dense(
            dense_d,
            activation="sigmoid",
            name="hidden_separate_branch_{0}_1".format(memo),
        )(state_features)
        q_values = Dense(
            num_actions,
            activation="linear",
            name="q_values_separate_branch_{0}".format(memo),
        )(hidden_1)
        return q_values

    def load_model(self, file_name):
        self.q_network = load_model(
            os.path.join(self.path_set.PATH_TO_MODEL, "%s_q_network.h5" % file_name)
        )
    ...
    def choose(self, count, if_pretrain):
        ...
        if if_pretrain:
            ...
            if (
                random.random() <= self.para_set.EPSILON
    ...
    def build_memory(self):
        # no memory ? what does this functio do???
        return []
    ...
    def build_network_from_copy(self, network_copy):
        ...
        network = model_from_json(
            network_structure, custom_objects={"Selector": Selector}
        )
    ...
    def _get_next_estimated_reward(self, next_state):
        ...
        if self.para_set.DDQN:
            a_max = np.argmax(
                self.q_network.predict(self.convert_state_to_input(next_state))[0]
            )
            next_estimated_reward = self.q_network_bar.predict(
                self.convert_state_to_input(next_state)
            )[0][a_max]
            return next_estimated_reward
        else:
            next_estimated_reward = np.max(
                self.q_network_bar.predict(self.convert_state_to_input(next_state))[0]
            )
            return next_estimated_reward
